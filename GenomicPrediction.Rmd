...
setwd("Q:/My Drive/Teaching/Teaching_videos")
library(tidyverse)
library(lme4)
library(emmeans)
library(rrBLUP)
...
dts = mu + site + rep(site) + block(rep:site) + entry + entry:env + residual  
  
We will fit rep and block as random effects and all else as fixed effects
```{r}
dts.mod = lmer("dts ~ 1 + site + (1|rep:site) + (1|block:rep:site) + sampleID + (1|sampleID:site)", pheno)
```
...
```{r}
dts.mod.rand = lmer("dts ~ 1 + site + (1|rep:site) + (1|block:rep:site) + (1|sampleID) + (1|sampleID:site)", pheno)
...
So, we want to estimate H = Vg/(Vg + Vge/9 + Verr/17) 
```{r}
Vg/(Vg + (Vge/9) + (Verr/17)) 
```
...
```
There is some indication of a number of markers on chromosome 10 (PZE.10) with larger effects (we know there is a major flowering time gene there).  
